...
class Header extends React.Component {
  constructor(props){
	...
	componentDidMount() {
		...
		if(role == 'student')	this.setState({link:'home', collapse:false});
	else this.setState({link:'search', collapse:false});
	}
	renderFullname(){
	...
	renderProfileDetailsHeader(){
		...
		return <Link onClick={(e)=>this.setActiveLink("none",e)} to={this.renderLinkRedirect()} className="nav-link" >
			      ...
			      }
			      {
			      	this.hasAuthorization('search') ? 
				   		<li className={"nav-item " + this.renderLinkActive("search")}>
				        	<Link onClick={(e)=>this.setActiveLink("search",e)}  to={window.basepath+'/search'} className="nav-link" href="#">Search</Link>
				      	</li>
				    : null
			      }
